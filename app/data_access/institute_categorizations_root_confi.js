export default function makeJobCategoryDB(docModels, sqMmodels) {
    ...
    async function findOne(_filter) {
        const query = await docModels.JobCategory.findOne(_filter)
    ...
    async function insert({ ...jobInfo }) {
        return docModels.JobCategory.create({ ...jobInfo })
    }

    async function insertMany(titles) {
        return docModels.JobCategory.insertMany(titles)
    }
    ...
    async function dbInfo() {
      const collectionName = await docModels.JobCategory.collection
        .collectionName;
