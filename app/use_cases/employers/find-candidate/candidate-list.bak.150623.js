export default function makeCandidateList({
    ...
    return async function candidateList({ query, user: { employerId } }) {
        ...
        if (!_filter.priority) {
            isInitSrch = true
            _filter.priority = PRIORITIES[jobCategory].map(x => x.id)
        }
        ...
        const candidates = await cacheSearchCandidateResultDB.aggregate([
            ...
            // },
            {
                $set: {
                    "data.TOTALSCORE": {
                        $add: ["$data.JTL_SCORE", "$data.LAY_SCORE", "$data.EXP_SCORE", "$data.NPD_SCORE", "$data.SAL_SCORE", "$data.SKL_SCORE"]
                    }
                }
            },
...

            {
                $project: {
                    ...
                    clusters: {
                        ...
                        salary: [
                            {
                                range: { min: "$prefMinSalary.salary", max: "$prefMaxSalary.salary" },
